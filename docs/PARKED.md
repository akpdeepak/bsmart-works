...
| ~~I01-S02~~ | ~~`workspace_id` column on the `events` table / `AppEvent`~~ | ~~I01-S04~~ | **RESOLVED in I01-S04** — V40 adds + backfills `workspace_id` and an append-only trigger (V40); `EventService.recordInWorkspace`. |
| ~~I01-S04 Event store~~ | ~~CI has no DB / Flyway stage~~ | ~~Process / CI~~ | **RESOLVED** — `backend-integration-test` CI job runs `FlywayMigrationIntegrationTest` (Testcontainers Postgres): boots the context, applies every migration end-to-end (catches dup versions / bad SQL), validates the schema, and asserts the append-only event trigger. Gates merges via the CI workflow. |
| I01-S04 | Remaining 15 event producers still call `EventService.record(...)` (workspace_id null on new rows until adopted) | Per domain spec (S05/S07/S09/…) | Switch each to `recordInWorkspace(...)` as that domain is refactored; existing rows already backfilled by V40. |
...
| I01-S01 | `App.jsx` is a ~5400-line monolith; only the auth surface is being extracted now | Tech-debt (`TECH-DEBT.md`) | Cross-cutting; affects every frontend spec — decompose deliberately. |
| I01-S06 Default types | Custom-type endpoints (`POST/PUT/DELETE /work-item-types`, `list` findAll) lack RBAC + workspace scoping (RB-40 §1; dormant in iter 1 — no custom types yet) | I03-S04 Custom WorkItem types | Harden with the custom-types feature; extract a service + tenant-scope + RBAC. |
| I01-S07 WorkItem CRUD | Extract a `WorkItemService` (RBAC + data access out of the 368-line `WorkItemController`) | Tech-debt / a later WorkItem pass | RBAC is already enforced in-controller; deferred to avoid a high-risk rewrite. The tenant-read leak (higher severity) was fixed in S07. |
