...
### Added
- Performance / KPI Framework (iteration 12, Cap L): layered metrics with privacy by design — Individual / Team / Project / Manager / Org views, with a metric catalog (11 seeded defaults), a safe custom-metric builder (sum/avg/percentile/count/ratio, no raw SQL), and immutable per-period snapshots (V44). Individual data is private by default; aggregated layers can never drill into an individual (a 403, not a hidden button), and thin aggregates are suppressed below a per-workspace anonymity floor. Includes voluntary individual sharing, a team-health composite + deterministic narrative, and a cycle-time distribution with outliers
