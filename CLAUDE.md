...
- **Flyway high-water mark:** **V120**. (V120 = smart_inbox_workspace_state — explicit notification
  workspace ownership plus durable per-workspace/user Inbox snooze and done state; legacy rows are
  backfilled only from sources with provable workspace ownership, and unresolved rows remain excluded.
  Additive, forward-only.) (V119 = webauthn_fido2_cose_columns — real WebAuthn/FIDO2
  (W1, EPIC-P1-webauthn-fido2 Slice WA1): adds nullable cose_credential/aaguid/fmt/user_handle/uv_initialized
  to webauthn_credentials and relaxes public_key_pem to nullable, so new FIDO2 registrations persist the
  authenticator's serialized AttestedCredentialData (COSE key) while legacy signed-nonce rows are untouched.
  Additive, forward-only — no backfill.)
  (V118 = rate_limit_windows — the shared fixed-window store for
  distributed rate limiting (W1 rate-limit PR3): an atomic UPSERT counts attempts using the DB clock so
  the limit holds across horizontally-scaled instances, replacing the per-JVM in-memory counter when
  app.rate-limit.distributed is on; default off → no behaviour change on merge. Additive, forward-only.)
  (V117 = revoked_tokens — a per-token (jti) blocklist for logout
  (W1 rate-limit/JWT PR2): /auth/logout and the portal equivalent record the presented token's jti, and
  the auth boundary rejects any blocklisted jti — fine-grained, single-session revocation alongside PR1's
  coarse per-subject cutoff; entries pruned past their token's exp. Additive, forward-only.)
  (V116 = idx_field_visibility_role_def — indexes the
...
- **Next migration:** **`V121__<description>.sql`**. *(Supersedes every stale lower-numbered reference.)*
