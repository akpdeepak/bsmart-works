...
mode model (`Today`, `Deliver`, `Service`, `Insights`, `Extend`) and the existing work-item hierarchy
as the baseline. Add only logically needed, backwards-compatible work-item hierarchy/type extensions
through explicit EPIC plans. If the local Downloads files are unavailable in a remote/GitHub-only
