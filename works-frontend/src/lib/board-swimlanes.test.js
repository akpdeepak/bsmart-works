...
describe('groupItemsIntoLanes', () => {
  it('returns no lanes for the flat mode', () => {
    expect(groupItemsIntoLanes(items, 'none', resolvers)).toEqual([]);
  });

  it('returns no lanes for an unknown dimension', () => {
  ...
  it('groups by assignee with the empty bucket last and resolved labels', () => {
    ...
    expect(lanes.map((lane) => lane.label)).toEqual(['Alice', 'Bob', 'Unassigned']);
    expect(lanes.find((lane) => lane.key === UNASSIGNED).items).toHaveLength(1);
  });

  it('groups by type alphabetically', () => {
    const lanes = groupItemsIntoLanes(items, 'type', resolvers);
    expect(lanes.map((lane) => lane.label)).toEqual(['Bug', 'Story']);
    expect(lanes[0].items).toHaveLength(2);
  });

  it('orders priority lanes by urgency', () => {
    const lanes = groupItemsIntoLanes(items, 'priority', resolvers);
    expect(lanes.map((lane) => lane.label)).toEqual(['CRITICAL', 'HIGH', 'LOW']);
  });

  it('groups by parent, resolving titles and bucketing parentless items last', () => {
