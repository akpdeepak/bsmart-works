...
### Added
- **Know Studio (Cap I)** — the Know section becomes one *living document* that absorbs what teams otherwise scatter across Confluence / SharePoint / Word / Excel / PowerBI / Miro, all persisted in the existing `articles.content_blocks` JSONB (no schema change) and rendered in read mode by a new `BlockRenderer` (block-format articles previously showed nothing when viewed):
  - **Excel** → `sheet` block with a pure, unit-tested formula engine (`=SUM/AVG/MIN/MAX/COUNT/PRODUCT/ROUND`, `A1:B3` ranges, `+ - * /` with precedence, circular-ref detection) and a Formulas/Values toggle
  - **PowerBI** → `chart` block (bar / line / pie) reusing the token-only design-system chart molecules, live preview from inline rows
  - **Miro / MS Whiteboard** → `whiteboard` block: sticky notes with pointer drag + keyboard nudge, token surfaces
  - **MS Word / Confluence** → `callout`, `quote`, `checklist`, `toggle` blocks; an **auto Table-of-Contents** generated live from headings; and a live **word-count / reading-time** status bar (no file to keep in sync; version history + inline comments + multi-author already replace Word's file-emailing/merge pain)
  - **SharePoint** → `bookmark` block; plus a Jira-style `workitem` reference block so knowledge stays linked to the work that produced it
  - **Live BQL widgets** → embed a dashboard widget inside an article (write BQL, pick group-by + measure + chart type: bar/pie/line/scorecard/table); renders pre-aggregated workspace data via the existing server-side pivot resolver — one BQL engine, tenant scope + field security enforced once (RB-10 §6, RB-40 §1). Several widgets = a dashboard in the page
  - **Visual delight** → a dependency-free emoji picker, a `sticker` block (big sizable emoji), and emoji on whiteboard sticky notes
  - **Any file in a page** → a `file` block referencing any file type by link (PDF / doc / sheet / image / zip / video) with a type-aware icon + inline image preview (native binary upload for articles is a tracked follow-up needing an article-attachments table/endpoint)
  - **Document from anywhere** → a reusable **"Save to Know"** quick-capture (lists spaces, creates a DRAFT article seeded with the captured content, links it back to the originating work item), wired into the work-item detail panel so knowledge can be captured without leaving the work
  - **Pervasive AI (RB-40 §2)** — every Know writing surface is AI-assisted, all routed through the AI Control Plane (scope/budget/cache/audit + mandatory deterministic fallback): an in-editor **AI compose bar** (write a paragraph from a prompt) and a per-block **Improve / Expand / Summarize / Shorten** menu (`POST /api/v1/knowledge/ai/compose`, workspace-scoped, RBAC `view_items`); a one-click **AI page summary** in the article header; an **"Ask your knowledge base"** RAG panel with citations + honest AI-provenance badges (reuses `/api/v1/ai/kb/ask`); and a reusable **AI writing assistant** (`AiTextAssist`) on the markdown editor and the comment box. AI affordances hide entirely when generation is not enabled for the workspace (most-restrictive-wins, server-resolved). Pure deterministic transforms double as the documented fallback and are unit-tested
- Iteration 18 — **Mobile + Real-time + Performance (Cap S)**: mobile-optimized **PWA** (service worker with offline app-shell + web-push handlers, installable, offline fallback page); **offline mode** — work-item drafts queued on-device and replayed on reconnect with optimistic-concurrency **conflict resolution** (`POST /api/v1/sync/work-item-drafts`); **real-time updates** over SSE that propagate workspace events to open clients (`GET /api/v1/realtime/stream`) plus **live co-presence** (`/api/v1/realtime/presence`); **push notifications** with per-event-type toggles, quiet hours, snooze, and P0-overrides-quiet (`/api/v1/push/preferences`, `/api/v1/push/subscriptions`, V53); **command palette** server-side fuzzy search across items + people (`/api/v1/command-palette/search`); **customizable keyboard shortcuts** (`/api/v1/shortcuts`) with a "?" help overlay; **performance SLAs** — per-operation P50/P95/P99 monitoring vs the RB-40 §5 budgets (`/api/v1/observability/performance`); and an **in-product status page** (`/api/v1/status`). Native iOS/Android are delivered as the install-free PWA in this monorepo. Every real-time/search path is workspace-scoped (RB-40 §1) with RBAC in the service layer (RB-10 §2).
- Enterprise Security + Compliance Certifications (iteration 19, Cap T, RB-40 §4) — SOC 2 Type 2 / ISO 27001 ready (V52):
  - **Passkeys / WebAuthn**: phishing-resistant, passwordless sign-in — registration + assertion ceremonies with single-use server challenges, ES256/RS256 public-key verification (`WebAuthnCrypto`), self-service credential management (`/api/v1/auth/passkeys`), and a pre-auth sign-in ceremony (`/api/v1/auth/passkey/authenticate/*`); password + TOTP MFA remain as the fallback (RB-40 §2)
  - **Conditional access policies**: IP allow-list (CIDR), geo, device-trust and time-of-day windows, per workspace and per role, most-restrictive-wins, enforced server-side (`/api/v1/security/conditional-access`)
  - **Tamper-evident audit log**: an append-only, SHA-256 hash-chained security audit trail (`AuditHashChain`) distinct from the domain event store — DB-enforced immutability (no UPDATE/DELETE), browse/filter/export, cryptographic chain verification, and streaming to external SIEM (Splunk / Datadog / ELK / webhook, JSON or CEF) (`/api/v1/security/audit-log`, `/streams`)
  - **Data residency + BYOK**: per-workspace residency region (IN/EU/US/AP/UK) and customer-managed-key *references* (AWS KMS / Azure KV / GCP KMS — key material never stored), AES-256 at rest (`/api/v1/security/settings`)
  - **AI anomaly detection on access**: new-geography, mass-export, privilege-escalation, impossible-travel and off-hours heuristics (`AnomalyDetector`, the deterministic fallback per RB-40 §2), with admin resolve/dismiss (`/api/v1/security/anomalies`)
  - **GDPR / DPDP data rights**: per-subject data export (portability) and right-to-be-forgotten erasure via crypto-shred / tokenization that preserves the immutable audit trail (RB-40 §3) (`/api/v1/security/data-requests`)
  - **Compliance program**: a pen-test / red-team / bug-bounty register and one-click SOC 2 Type 2 / ISO 27001 evidence bundles that snapshot live control coverage (`/api/v1/security/pentests`, `/evidence`)
  - **Security Center** (frontend, Cap T): an admin surface — posture overview, audit-log explorer with chain-verified badge + CSV export, conditional-access list, anomaly queue, data residency/BYOK controls, pen-test register, evidence-bundle generation, and self-service passkeys (a WebCrypto software authenticator that signs the server challenge end-to-end) — reachable from the sidebar; new `securityClient` + `passkey` libs through the single `apiClient`
- Universal Customization Engine (iteration 17, Cap R): one configuration framework so admins tune every workspace behavior without engineering tickets. **One unified versioned JSON document per workspace** holds branding/locale/timezone/working-calendar/defaults, custom forms, custom pages, code-extension definitions and the set of locked paths — so versioning (append-only history), diff, rollback, reusable templates (internal + customer-shareable, with a seeded `Utility Customer Workspace` base), sandbox-before-promote, JSON/YAML import-export, **owner-only lockable settings** (enforced in `ConfigService`, not the UI — RB-40 §1), and pre-apply **impact analysis** ("affects N items, M users, K automations") all apply uniformly across the whole surface. Every change funnels through one mutation path → versioned + audited (`CONFIG_UPDATED` in the immutable event log) + lock-checked. Extension *execution* is deferred to a sandboxed runtime (security — TD-015). APIs at `/api/v1/config` (settings, versions, diff, rollback, impact, export, import, extension-points), `/api/v1/config/templates`, `/api/v1/config/sandboxes` (V51); frontend at `Configure → Customization`
...
- Iteration 20 — **Advanced AI (Cap O)**: workspace-defined **custom assistants** (named personas, e.g. a Compliance Assistant) with persona- and memory-grounded chat; **multi-step AI agents** that plan a goal into ordered capability steps and run them as an audited `AiAgentRun` (read-only suggestion artifact); **AI memory/context** remembered across sessions per (workspace, user); and **conversational dashboards** (natural language → structured widget spec) — all at `/api/v1/ai/{assistants,agents,memory,conversational-dashboards}`, every model call routed through the AI Control Plane with a documented deterministic fallback (V54)
- Iteration 20 — **App Marketplace foundation + Developer Portal (Cap R)**: a global, browsable extension catalogue with per-workspace installs and server-side **permission scoping** (granted scopes ⊆ requested), plus a developer-portal SDK manifest + sandbox credentials — at `/api/v1/marketplace` and `/api/v1/developer-portal` (V55)
- Iteration 20 — **Advanced Knowledge (Cap I)**: workspace-scoped **document templates** (markdown skeletons with placeholders), **multi-author collaboration** (article author/co-author/reviewer roster), and AI **structured-data extraction** (deterministic regex/keyword fallback) — at `/api/v1/knowledge/{templates,articles/{id}/authors,extract}` (V56)
- Iteration 20 — **Customer chat support (Cap N)**: real-time portal chat with **AI tier-1 auto-response and human escalation** (canned holding reply + auto-escalate on fallback or on an explicit human request), an agent **Support Inbox**, and a customer-portal chat widget — at `/api/v1/support-chat` (+ `/portal`) (V57)
- Iteration 20 — **Localization (Cap A)**: a dependency-free i18n layer with a **10-language** catalogue (English, Hindi, Spanish, French, German, Portuguese, Japanese, Mandarin, Arabic, Korean), English fallback, **RTL** support (Arabic), a persisted per-user locale preference (`PUT /api/v1/users/me/locale`, V58), and a top-bar language switcher; the primary navigation is translated end to end
- Iteration 20 — **Performance hardening (Cap S)** and **final security hardening (Cap T)**: composite indexes for the hottest multi-column query patterns (V58), a documented load-test plan against the RB-40 §5 NFR budgets (`PERFORMANCE.md`), a security disclosure / bug-bounty policy (`SECURITY.md`), and a WCAG 2.2 AA accessibility audit (`ACCESSIBILITY.md`)
