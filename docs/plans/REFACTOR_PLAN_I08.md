...
- **Developer:** new migration `V38` (forward-only); RBAC via `manage_sla` (LEAD tier) in the
...
## Risk & rollback
- `V38` is forward-only and additive (new tables only); rollback is a new forward migration.
