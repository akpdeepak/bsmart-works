...
| Icons | `lucide-react` |
| API client | Single `apiClient` wrapper — `works-frontend/src/lib/apiClient.js` (built; no inline fetch/axios per component) |
...
- **Frontend is a monolith.** `works-frontend/src/App.jsx` is ~4,200 lines and holds most
  screens + state — it contradicts the "render one thing" rule in §3. A feature-module split is
  planned; until then, extend it carefully and prefer pulling shared bits into
  `components/works/` rather than growing `App.jsx` further.
...
### Database (Flyway — current high-water mark: **V26** on `main`; note V16 and V23 were skipped)
...
- Next migration is **`V27__<description>.sql`**. Naming: `V{n}__{snake_case_description}.sql`.
  (Recent: V18 project_slugs, V19 data_quality_cleanup, V20 drop_dead_event_log,
  V21 iteration3_workflows_fields_permissions, V22 iteration4_pm_artifacts,
  V24 knowledge_repository, V25 releases_and_worklogs, V26 cross_project_dependencies.)
- **New tables are PLURAL** (`work_items`, `users`, `releases`, `worklogs`, …).
  ⚠️ **Known convention break:** the V21/V22 tables are **singular** — `workflow`,
  `workflow_status`, `workflow_transition`, `field_def`, `work_item_field_value`,
  `permission_scheme`, `role_def`, `role_permission`, `field_visibility`,
  `work_item_type_config`, `field_layout`, `wiql_filter` (V21); `risk`, `assumption`,
  `pm_issue`, `dependency`, `decision`, `meeting`, `meeting_note`, `action_item`,
  `stakeholder`, `lesson_learned` (V22). Match the existing name when touching those tables;
  use **plural** for any NEW table. Don't rename without a migration.
- **Verified plural tables (core + V24–V26):**
  ...
  password_reset_tokens, events, knowledge_spaces, articles, article_versions,
  article_work_item_links, releases, work_item_releases, worklogs, cross_project_dependencies`
...
- Error shape `{ code, message, field? }` via one `@ControllerAdvice` (implemented:
  `GlobalExceptionHandler` + typed `ApiException`).
...
- Dark mode: implemented via Tailwind `dark:` variants throughout. Every new surface must
  define both light and `dark:` token classes — don't ship light-only UI.

### Components (Current Reality — `works-frontend/src/components/works/`)
Built so far (verified):
- `Button` — `button.jsx` (cva variants: primary/secondary/ghost/danger/action/link)
- `Logo` — `logo.jsx`
- `StatusBadge` — `status-badge.jsx`
- `RichTextEditor` — `rich-text-editor.jsx`
- Shared primitives — `ui.jsx`: `Avatar`, `TypeBadge`, `PriorityBadge`, `RoleBadge`, `StatCard`,
  `EmptyState`, `NavItem`, `Modal`, `Field` (plus helpers `getInitials` / `renderMd` /
  `formatEventType` and the `TYPES` / `ROLE_CONFIG` / `PRIORITY_CONFIG` maps).

When adding components: follow the `button.jsx` pattern (cva + `cn()`), use token classes only
(light + `dark:`), and add to this folder. The brand doc defines a larger target inventory —
build toward it incrementally; do not assume it all already exists.
...
**Current status (verified against migrations V1–V26 + `ITERATION-0x-COMPLETE.md` on `main`):**
Iterations **1–4 are complete** — work items, sprints, workflows/custom-fields/permissions/WIQL,
and PM artifacts (RAID/decisions/meetings/actions). **Iteration 5** (Knowledge + Releases +
Worklogs) has the **backend landed** (V24/V25 + controllers) but **no frontend UI yet**.
Iterations 6–20 not started. Confirm the active iteration with Deepak before building forward.
...
- Don't use singular names for NEW tables — convention is plural. (V21/V22 tables are singular;
  match them when editing those, but make new tables plural — see §3.)
...
- Don't assume the full target component library exists — see §4 for what's actually built.
...
- [ ] New tables/columns have a Flyway migration (`V27+`), plural names for new tables
...
## 8. How These Rules Stay True (Sync & Enforcement)

**This file is the single source of truth.** The per-tool rule files are *generated* from it —
never hand-edit them. Editing `CLAUDE.md` is the only manual step: the Husky pre-commit hook runs
`scripts/generate-ai-rules.mjs`, which regenerates and **auto-stages**
`.github/copilot-instructions.md` (Copilot), `.windsurfrules` (Windsurf), `AGENTS.md`
(cross-tool), and `.cursor/rules/bsmart.mdc` (Cursor). CI (`ai-rules-in-sync`) re-runs it with
`--check`, so nothing merges out of sync even if the hook is bypassed.

**Machine-enforced** — a violation fails the build regardless of which tool or person wrote the
code (pre-commit + CI):
- **ESLint** (`works-frontend/eslint.config.js`, severity `error`): raw hex in Tailwind
  arbitrary values, `works-*` token names, inline `fetch()`, `axios` import. (§3, §4)
- **`scripts/guardrails.sh` BLOCK:** `works-*` token names (§4) · new `com.bcits.works.*`
  packages (§3) · RBAC annotations in controllers (§3) · Flyway filename format (§3) ·
  **new tables must be plural** (§3).
- **CI also:** `generate-ai-rules.mjs --check`, frontend `npm run build`, backend compile +
  Checkstyle.

**Soft / baseline-debt** (`guardrails.sh` WARN — reported, non-blocking until the `App.jsx`
baseline is clean, then `guardrails.sh --strict`): broad raw-hex scan · arbitrary px/rem
spacing · inline fetch/axios scan.

**Guidance-only** (everything else here — enforced by review, not a gate): `@Valid` DTO-first,
AI-feature fallbacks, scope discipline, comments-explain-why, iteration discipline, "render one
thing," event-sourcing. *If you add a rule that can be checked mechanically, wire it into
`eslint.config.js` or `guardrails.sh` and cite the section here — keep the doc and the gate in
lockstep.*

> Companions (kept consistent with this file): [`docs/ENGINEERING-PRINCIPLES.md`](docs/ENGINEERING-PRINCIPLES.md)
> (the *why*) · [`CONTRIBUTING.md`](CONTRIBUTING.md) (setup) ·
> [`.github/pull_request_template.md`](.github/pull_request_template.md) (PR checklist).

---

*Verified against the codebase on 2026-06-01 (migrations V1–V26, `components/works/`, `pom.xml`,
`package.json`). Source specs: Capability Map v3.5, Complete Iteration Guide, Tech Stack &
Architecture, Brand & Identity (bSmart Works master package). Where spec and code conflict, code
is canonical and the conflict is flagged ⚠️.*
