...
public final class NavSurfaces {
    ...
    // Ordered for stable, readable responses.
    private static final Map<String, Integer> MIN_TIER = new LinkedHashMap<>();
    static {
        // Home
        MIN_TIER.put("dashboard", 1); MIN_TIER.put("myworks", 2); MIN_TIER.put("notifications", 2);
        // Deliver
        MIN_TIER.put("smcockpit", 3); MIN_TIER.put("board", 1); MIN_TIER.put("backlog", 2);
        MIN_TIER.put("sprint", 2); MIN_TIER.put("releases", 2); MIN_TIER.put("projects", 1);
        MIN_TIER.put("pm", 3);
        // Insight
        MIN_TIER.put("reports", 1); MIN_TIER.put("dashboards", 1); MIN_TIER.put("reportbuilder", 3);
        MIN_TIER.put("performance", 3);
        // Service
        MIN_TIER.put("service", 2); MIN_TIER.put("supportinbox", 2); MIN_TIER.put("sla", 2);
        MIN_TIER.put("compliance", 4);
        // Know
        MIN_TIER.put("knowledge", 1); MIN_TIER.put("knowledgeadvanced", 2);
        // Extend
        MIN_TIER.put("automations", 3); MIN_TIER.put("integrations", 4); MIN_TIER.put("aistudio", 4);
        MIN_TIER.put("marketplace", 4); MIN_TIER.put("developerportal", 4);
        // Set up
        MIN_TIER.put("workspace", 4); MIN_TIER.put("settings3", 4); MIN_TIER.put("aicontrol", 4);
        MIN_TIER.put("customization", 4); MIN_TIER.put("security", 5); MIN_TIER.put("trash", 3);
        // Satellite cockpits + BQL
        MIN_TIER.put("developer", 2); MIN_TIER.put("poworkspace", 3); MIN_TIER.put("leadership", 4);
        MIN_TIER.put("adminops", 4); MIN_TIER.put("bql", 2);
    }
