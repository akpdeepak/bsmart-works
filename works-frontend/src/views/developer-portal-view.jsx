...
export default function DeveloperPortalView({ workspaceId }) {
  ...
  // Kept separate from `error`: a failed sandbox generation must not replace the whole portal with
  // an error state, which is what routing it through AsyncBoundary would do. Load failures own the
  // boundary; action failures stay an inline banner above still-rendered content.
  ...
  async function generateSandbox() {
    ...
    try {
      ...
      setActionError(e.message || 'Could not generate sandbox credentials.');
    } finally {
      setGenerating(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      <AsyncBoundary
        ...
        error={error}
        onRetry={load}
        errorTitle="Couldn't load the developer portal"
        empty={!sdk}
        emptyIcon={Code2}
        emptyTitle="Developer portal unavailable"
        emptySubtitle="The SDK manifest could not be loaded for this workspace. Refresh the page, or contact a workspace admin if the problem persists."
        label="Loading developer portal"
        className="space-y-3"
        skeleton={[0, 1, 2].map((i) => (
          <div key={i} className="h-24 rounded-xl bg-neutral-100 dark:bg-neutral-800 animate-pulse" />
        ))}
      >
        {/* Guarded rather than relying on `empty`: JSX children are evaluated when the element is
            created, not when AsyncBoundary decides to render them, so `sdk.sdkVersion` below would
            throw on a null sdk before the empty branch was ever reached. */}
        {sdk && (
          <>
