...
### TD-016 — Nav visibility tier map is a client-side parallel to server RBAC
- **What:** Role-based nav visibility (`works-frontend/src/lib/nav-model.js` → `SURFACE_TIER`) decides which surfaces appear in the rail / sub-rail / ⌘K by mapping each surface to a minimum tier (VIEWER…OWNER). This is a *second* copy of access intent; the authoritative one is the server's `permissions.min_tier` enforced by `RbacService`.
- **Why accepted:** There is no endpoint that returns "surfaces this user may see," and the surface→permission mapping isn't 1:1 (a surface uses several permissions). A pragmatic client tier map ships the requested role-based declutter now. It is **not** a security boundary — every query/action is still authorised server-side (RB-10 §2, RB-40 §1), so a stale entry only mis-declutters the menu, never leaks data.
- **Impact:** The client map can drift from server permissions; a surface could be shown to a tier the server then 403s (or hidden from one that is actually allowed). Cosmetic, not a breach.
- **Trigger:** Add a `GET /rbac/me/surfaces` (or fold a `visibleSurfaces` array into `/rbac/me`) that the server derives from the real permission tiers, and drive the nav from it — retiring `SURFACE_TIER`. Do this when surface/permission mappings start changing often or before external tenants self-serve roles.
