...
Migration **V50**. Frontend: **AI Studio** view (Assistants · Agents · Ask), each reply badged with
...
Migration **V51**. Frontend: Marketplace + Developer Portal views.
...
Migration **V52**. Frontend: Advanced-Knowledge templates + extraction view.
...
Migration **V53**. Endpoints `/api/v1/support-chat` (agent) + `/portal` (customer).
...
language switcher. The primary navigation is translated end to end. Migration **V54** (locale column).
...
- **Performance:** composite indexes for the hottest query shapes (`work_items(project_id,status)`,
  ...
  RB-40 §5 NFR budgets are documented in **`PERFORMANCE.md`**. Migration **V54**.
...
- **Gate:** backend unit suite + JaCoCo coverage green; all blocking guardrails pass; frontend lint +
  ...
  numbers (V50–V54), and valid PostgreSQL (applied end-to-end by the Testcontainers CI job).

Orchestrator §6 updated: **active iteration 20 (complete)**, Flyway high-water mark **V54**, next
migration **V55**. The generated AI-rules files were regenerated from `ai-rules/` and are in sync.
