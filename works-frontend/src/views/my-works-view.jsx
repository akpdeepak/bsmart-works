import { User, Star, AtSign, ClipboardList } from 'lucide-react';
...
// My Works view — extracted from the App.jsx monolith (UX finding A3/H2). Behaviour-preserving:
// the parent owns the item/notification data, the active tab, and the selection handlers.
        ...
        <div className="space-y-2">
          {workItems.filter(i => i.createdBy === currentUser.id || i.assigneeId === currentUser.id).slice(0, 20).map(i => (
            <div key={i.id} onClick={() => setSelectedItem(i)} role="button" tabIndex={0} onKeyDown={onPressKey}
              ...
              <span className="text-xs font-mono text-neutral-600 dark:text-neutral-400">{i.id}</span>
