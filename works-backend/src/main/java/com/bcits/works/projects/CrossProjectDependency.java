...
@Filter(name = WorkspaceFilterActivator.FILTER_NAME,
        condition = "(from_project_id IN (SELECT p.id FROM projects p WHERE p.workspace_id = :workspaceId) OR to_project_id IN (SELECT "
                + "p.id FROM projects p WHERE p.workspace_id = :workspaceId))")
