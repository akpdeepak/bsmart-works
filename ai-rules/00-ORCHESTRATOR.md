...
- **Flyway high-water mark:** **V68** (`V68__add_updated_at_to_workspaces_and_projects.sql`; note: V16 was
  ...
  (boot unblock); V67 = OAuth credentials + AES-256 encryption + PII vault; V68 = updated_at on
  workspaces + projects, unblocks WorkspaceTenantIsolationIT + ComplianceEvaluationPerformanceTest).
- **Next migration:** **`V69__<description>.sql`**. *(Supersedes every stale lower-numbered reference.)*
