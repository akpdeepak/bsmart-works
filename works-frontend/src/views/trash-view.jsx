...
export default function TrashView({ loading = false, trashItems, restoreFromTrash, permanentDelete }) {
  if (loading && trashItems.length === 0) {
    return (
      <PageLayout>
        <AsyncBoundary loading skeleton={<><Skeleton className="h-7 w-24 mb-6" /><ListSkeleton rows={4} /></>} />
      </PageLayout>
    );
  }
  ...
  return (
    <PageLayout title="Trash" description="Deleted items are kept for 30 days">
      <AsyncBoundary
        ...
        emptySubtitle="Deleted work items will appear here for 30 days before permanent removal."
      >
