...
public interface ArticleRepository extends JpaRepository<Article, String> {
    ...
    List<Article> findAllScopedToUser(@Param("userId") String userId);
    ...
    @Query(nativeQuery = true,
           value = "SELECT a.* FROM articles a " +
                   ...
                   "ORDER BY a.updated_at DESC")
    List<Article> findBySpaceIdScopedToUser(@Param("spaceId") String spaceId,
    ...
    @Query(nativeQuery = true,
           value = "SELECT a.* FROM articles a " +
                   ...
                   "AND ks.workspace_id IN (SELECT workspace_id FROM workspace_members WHERE user_id = :userId) " +
                   "ORDER BY a.updated_at DESC")
    List<Article> findByTitleScopedToUser(@Param("query") String query,
                                           @Param("userId") String userId);
}
