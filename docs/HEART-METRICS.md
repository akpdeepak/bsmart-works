...
payload: {
  ...
  days_since_workspace_creation: <integer>
}
```
...
```
[Step 1] WORKSPACE_CREATED
         → A new workspace is created (or accepted via invite).

[Step 2] WORKSPACE_TEMPLATE_APPLIED  (or WORKSPACE_SETUP_SKIPPED)
         → A project template (Scrum / Kanban / Bug / RAID) is applied,
           or the user explicitly skips the setup wizard.

[Step 3] WORKSPACE_FIRST_VALUE       ← canonical "first value" event (§3)
         → First real work item created in a real project.

[Step 4] WORKSPACE_TEAMMATE_INVITED
         → At least one workspace invitation is sent to a new user.

[Step 5] WORKSPACE_DAY_2_RETURN
         → Any workspace member performs a meaningful action on calendar day 2
           after workspace creation (create / update / comment / plan).
```
...
Before WI-09 (instrumentation) begins, confirm:

- [ ] `EventService.emit()` accepts the event types in §4 (or they can be added without schema
      change — they are application-layer event types, not new DB columns).
- [ ] The `events` table has capacity for the additional event volume (estimate: 10–50 events per
      active workspace per day — negligible at current scale).
- [ ] The `bql_run_audit` table (V85) is the source for BQL engagement events — WI-09 reads from
      it, does not double-write.
- [ ] The workspace-creation timestamp is available (`workspaces.created_at`) — needed for
      funnel step timing and `days_since_workspace_creation` payload field.
- [ ] **Deepak sign-off on WI-09** (⚠️) before any instrumentation code is written — per
      Orchestrator §5 (data model / audit / DPDP changes require explicit sign-off).
